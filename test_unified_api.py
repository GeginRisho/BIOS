# BIOS Unified API Endpoint Operational Verifier
# Executes active integration checks routed through the unified single-process FastAPI gateway on port 8000.
...
async def test_unified_api():
    print("==========================================")
    print("VERIFYING BIOS UNIFIED API GATEWAY ROUTING...")
    print("==========================================")
    
    # 1. AUTH SERVICE - Register -> Login -> Me
    print("\n1. Testing Auth Service /api/v1/auth [JWT Authentication]...")
    async with httpx.AsyncClient() as client:
        test_email = "verifier_unified@bios.global"
        ...
        reg_resp = await client.post(f"{BASE_URL}/api/v1/auth/register", json=reg_payload)
        if reg_resp.status_code in (200, 201, 400):
            print("  [OK] /register endpoint responded successfully.")
        else:
            print(f"  [FAIL] /register status: {reg_resp.status_code} body: {reg_resp.text}")
            return
            
        # Login
        login_payload = {"email": test_email, "password": test_pwd}
        login_resp = await client.post(f"{BASE_URL}/api/v1/auth/login", json=login_payload)
        if login_resp.status_code == 200:
            token_data = login_resp.json()
            access_token = token_data["access_token"]
            print("  [OK] /login endpoint authenticated credentials. Token obtained.")
        else:
            print(f"  [FAIL] /login status: {login_resp.status_code} body: {login_resp.text}")
            return
    ...
    print("\n2. Testing Business Service /api/v1/businesses [Database Connectivity]...")
    async with httpx.AsyncClient() as client:
        biz_payload = {
            "name": "Planetary Logistics Unified",
            "website": "https://logistics.bios",
            "industry": "Transportation",
            "country": "Germany",
            "city": "Hamburg"
        }
        biz_resp = await client.post(f"{BASE_URL}/api/v1/businesses", json=biz_payload)
        if biz_resp.status_code in (200, 201):
            biz_data = biz_resp.json()
            biz_id = biz_data["id"]
            print(f"  [OK] Business Twin registered in SQLite database. ID: {biz_id}")
        else:
            print(f"  [FAIL] /businesses post status: {biz_resp.status_code} headers: {dict(biz_resp.headers)} body: {biz_resp.text}")
            return

    # 3. KNOWLEDGE GRAPH - D3 Data structures
    print("\n3. Testing Knowledge Graph /api/v1/graph [Cypher Nodes Mapping]...")
    async with httpx.AsyncClient() as client:
        kg_resp = await client.get(f"{BASE_URL}/api/v1/graph/d3")
    ...
    print("\n4. Testing Digital Twin Service /api/v1/twin [Reputation Scoring]...")
    async with httpx.AsyncClient() as client:
        dt_resp = await client.post(f"{BASE_URL}/api/v1/twin/compute/{biz_id}")
    ...
    print("\n5. Testing Prediction Service /api/v1/predictions [Prophet Extrapolator]...")
    async with httpx.AsyncClient() as client:
        pred_resp = await client.post(f"{BASE_URL}/api/v1/predictions/forecast/{biz_id}")
    ...
    print("\n6. Testing Simulation Service /api/v1/simulations [Macro Recession Shocks]...")
    async with httpx.AsyncClient() as client:
        sim_payload = {
            "business_id": biz_id,
            "time_horizon": "1Y",
        ...
        sim_resp = await client.post(f"{BASE_URL}/api/v1/simulations/run", json=sim_payload)
    ...
    print("\n7. Testing Agent Swarm Service /api/v1/agents [16-Agent Coordinator Swarm]...")
    async with httpx.AsyncClient() as client:
        agent_payload = {"message": "Assess competitors overlaps for apple"}
        agent_resp = await client.post(f"{BASE_URL}/api/v1/agents/chat", json=agent_payload, timeout=20.0)
    ...
    print("\n8. Testing Search Service /api/v1/search [Lexical-Vector Fusion]...")
    async with httpx.AsyncClient() as client:
        search_resp = await client.get(f"{BASE_URL}/api/v1/search/hybrid?q=planetary", timeout=20.0)
    ...
    print("\n9. Testing Report Service /api/v1/reports [SWOT Briefing Templates]...")
    async with httpx.AsyncClient() as client:
        rep_resp = await client.get(f"{BASE_URL}/api/v1/reports/export/{biz_id}")
    ...
    print("\n10. Testing Notification Service /api/v1/notifications [WebSocket Listener]...")
    
    # Trigger socket listener in background, then broadcast rest frame alert
    ws_uri = f"ws://127.0.0.1:8000/api/v1/notifications/ws"
    try:
        async with websockets.connect(ws_uri) as websocket:
            ...
            async with httpx.AsyncClient() as client:
                ...
                alert_resp = await client.post(f"{BASE_URL}/api/v1/notifications/broadcast", json=alert_payload)
    ...
    print("\n==========================================")
    print("ALL UNIFIED ENDPOINTS VERIFICATIONS SUCCESSFUL!")
    print("==========================================")
